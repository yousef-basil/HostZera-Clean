...
<div class="max-w-lg mx-auto">
    <div class="mb-6">
        <a href="{{ route('admin.categories.index') }}" class="text-blue-500 hover:underline text-sm font-medium">
            <i class="fas fa-arrow-left mr-1"></i> Back to Categories
        </a>
        <h1 class="text-2xl font-bold text-gray-800 mt-2">Update Service Category</h1>
    </div>

    <div class="bg-white rounded-xl shadow-sm p-8 border border-gray-100">
        <form action="{{ route('admin.categories.update', $category) }}" method="POST" class="space-y-5" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="description" rows="2"
                       ...
                       placeholder="e.g. Best for small websites and blogs...">{{ old('description', $category->description) }}</textarea>
            ...
            <div>
                ...
                <textarea name="hero_description" rows="3"
                       class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                       placeholder="e.g. Run Odoo ERP on dedicated, optimized servers...">{{ old('hero_description', $category->hero_description) }}</textarea>
            ...
            <div>
                ...
                <textarea name="features" rows="3"
                       class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                       placeholder="Best for professional use&#10;99.9% Uptime Guarantee&#10;24/7 Expert Support">{{ old('features', $category->features) }}</textarea>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="col-span-2 sm:col-span-1">
                    ...
                    <div class="grid grid-cols-4 gap-2">
                        ...
                        <label class="relative cursor-pointer">
                            <input type="radio" name="icon" value="{{ $iconKey }}" class="peer sr-only" {{ $category->icon == $iconKey ? 'checked' : '' }}>
                ...
                <div>
                    ...
                    @if($category->icon_image)
                        <div class="mb-2">
                            <img src="{{ asset('storage/' . $category->icon_image) }}" alt="Icon" class="w-10 h-10 object-contain">
                        </div>
                    @endif
